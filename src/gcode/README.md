...
identifies us as a PrusaSlicer-family slicer from the
`; generated by <name> on <date>` line, then scans the **last 1 MiB** for
`; key = value` comments — which is why the block is a footer, not part of the
header. OctoPrint reads the same shape.
