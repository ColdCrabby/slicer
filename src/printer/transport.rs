//! Native HTTP transport for printers — probing, status and G-code upload.
//!
//! Interpretation of what a Klipper host reports lives in [`super::klipper`];
//! this module is only the wire. See [`super`] for why the request runs here
...
use super::detection::PrinterDetection;
...
/// Probe Moonraker and read the machine out of its own configuration.
///
/// Four stages, cheapest and most load-bearing first, each optional after the
/// identifying one. `configfile` is last because it is a copy of the whole
/// `printer.cfg` and can run to megabytes on a macro-heavy setup; by the time
/// we ask for it the bed volume and every *presence* question is already
/// answered, so a host that times out here still yields a usable profile.
async fn detect_moonraker(
    info_client: &reqwest::Client,
    detect_client: &reqwest::Client,
    base: &str,
) -> Option<PrinterDetection> {
    let resp = info_client
        .get(format!("{base}/printer/info"))
        .send()
        .await
        .ok()?;
    ...
    if !looks_like_moonraker(&info["result"]) {
        return None;
    }

    let mut probe = KlipperProbe::default();
    probe.absorb_info(&info["result"]);

    // Bed spans and the live motion limits — a tiny payload, and the one thing
    // a profile cannot be built without.
    if let Some(body) = query(detect_client, base, "/printer/objects/query?toolhead").await {
        probe.absorb_objects(&body["result"]["status"]);
    }
    // Which optional Klipper modules and macros exist. Names only, so this
    // stays small even where the config behind them does not.
    if let Some(body) = query(detect_client, base, "/printer/objects/list").await {
        probe.absorb_object_list(&body["result"]);
    }
    // Saved bed meshes, so the wizard can offer them by name.
    if let Some(body) = query(detect_client, base, "/printer/objects/query?bed_mesh").await {
        probe.absorb_objects(&body["result"]["status"]);
    }
    // The configured values themselves.
    if let Some(body) = query(detect_client, base, "/printer/objects/query?configfile").await {
        probe.absorb_objects(&body["result"]["status"]);
    }

    Some(probe.into_detection())
}

/// GET a Moonraker endpoint and parse its JSON, or `None` on any failure.
///
/// Every enrichment stage is best-effort: a refusal, a timeout or a malformed
/// body costs that stage's findings and nothing more.
async fn query(client: &reqwest::Client, base: &str, path: &str) -> Option<serde_json::Value> {
