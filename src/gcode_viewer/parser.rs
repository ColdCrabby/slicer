...
/// Comment key carrying the whole-print time estimate, in the two spellings the
/// generator emits (the bare one in the metadata header, the `(normal mode)`
/// one in the PrusaSlicer-compatible block). Matching both also reads files
/// produced by PrusaSlicer and OrcaSlicer, which use the same key.
const ESTIMATED_TIME_KEY: &str = "estimated printing time";

/// The whole-print time estimate the file's own header advertises, in seconds.
///
/// Deliberately a separate pass over the text rather than another arm of
/// [`process_comment`]: this is one number about the *file*, not per-layer
/// state threaded through the geometry parse, and keeping it out means the
/// layer parser's signature — and its dozen call sites — stay as they are.
///
/// Returns `None` when the file carries no such comment, which is the honest
/// answer for G-code from a source that never estimated one.
pub(super) fn parse_estimated_print_time_s(bytes: &[u8]) -> Option<f32> {
    ...
    for line in text.lines() {
        ...
        if !comment.to_ascii_lowercase().starts_with(ESTIMATED_TIME_KEY) {
            continue;
        }
        let Some((_, value)) = comment.split_once('=') else {
            continue;
        };
        if let Some(seconds) = parse_human_duration_s(value) {
            return Some(seconds);
        }
    }
    None
...
/// writers of this comment do not agree: ours emits `1h 2m 3s`, and a file from
/// elsewhere may carry `1d0h5m`. A value with no recognised unit at all is
/// rejected rather than guessed at.
fn parse_human_duration_s(value: &str) -> Option<f32> {
    ...
    for ch in value.trim().chars() {
        ...
        let multiplier = match ch.to_ascii_lowercase() {
            ...
            's' => 1.0,
            _ => {
                // Whitespace between fields is expected; anything else ends a
                // value we no longer understand.
                if !ch.is_whitespace() {
                    return None;
                }
                continue;
...
";
    ...
    #[test]
    fn reads_the_estimate_from_our_own_header() {
        let gcode = "; generated by Cold Crabby\n; estimated printing time = 1h 2m 3s\nG1 X0\n";
        assert_eq!(parse_estimated_print_time_s(gcode.as_bytes()), Some(3723.0));
    }

    #[test]
    fn reads_the_prusaslicer_spelling_too() {
        let gcode = "; estimated printing time (normal mode) = 2m 30s\n";
        assert_eq!(parse_estimated_print_time_s(gcode.as_bytes()), Some(150.0));
    }

    #[test]
    fn takes_the_first_estimate_when_header_and_footer_both_carry_one() {
        let gcode = "; estimated printing time = 10s\nG1 X0\n; estimated printing time = 99s\n";
        assert_eq!(parse_estimated_print_time_s(gcode.as_bytes()), Some(10.0));
    }

    #[test]
    fn accepts_unspaced_fields_and_a_day_component() {
        assert_eq!(
            parse_estimated_print_time_s(b"; estimated printing time = 1d0h5m\n"),
            Some(86_700.0)
        );
    }

    #[test]
    fn reports_nothing_when_the_file_never_estimated_one() {
        assert_eq!(parse_estimated_print_time_s(b"G1 X0 Y0\n"), None);
        // A key with a value in no unit we know is not worth a guess.
        assert_eq!(
            parse_estimated_print_time_s(b"; estimated printing time = unknown\n"),
            None
        );
    }
}
