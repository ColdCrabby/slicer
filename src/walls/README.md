...
| [`fuzzy_skin::apply`](fuzzy_skin.rs) | Cosmetic outer-wall texture, gated on `SlicingParams::fuzzy_skin`. Called by [`crate::core::pipeline`], not by `generate_walls` — see below. |
...
outer-wall geometry and never feeds back into interior-region, surface or
infill math. It carries along any per-vertex widths flow compensation already
wrote, and never touches skirt/brim (also tagged `OuterWall`, but generated by
adhesion, which runs after it).

It also fuzzes `OverhangPerimeter` and **open** `OuterWall` arcs — the pieces
`classify_overhang_perimeters` leaves behind when it splits a loop at an air
boundary — leaving only their two endpoints untouched, since an adjacent run
from the same split shares that exact coordinate and jittering it would tear
a gap. Skipping open arcs entirely was the original (buggy) behaviour: it left
straight, un-textured segments wherever a wall happened to cross unsupported
air next to a fuzzed one.
