...
/// Show one of an entry's files in Finder or Explorer, selected.
///
/// Only a path the library already records is revealed: the webview names a
/// location it was given, never an arbitrary one. iOS has no way to point the
/// Files app at a file, so it refuses there.
#[tauri::command]
pub async fn library_reveal(path: String) -> Result<(), String> {
    let known = off_thread({
        let path = path.clone();
        move || {
            Ok(library()
                .load()?
    ...
    .await?;
    if !known {
        return Err("not a file in the library".into());
    }
    reveal_in_file_manager(std::path::Path::new(&path)).map_err(|e| e.to_string())
}
...
/// No Linux file manager agrees on a "select this file" flag, so open its folder.
...
#[cfg(target_os = "ios")]
fn reveal_in_file_manager(_path: &std::path::Path) -> std::io::Result<()> {
