...
apply_dimensional_compensation()     — XY size + medial-limited elephant foot, on the raw contours
...
### Dimensional compensation — correct the contour, never the bead

[src/core/compensation.rs](src/core/compensation.rs) runs **first**, on the raw
`slice_mesh` output, and that placement is the whole design. Walls, interior,
surfaces and infill are all derived from those contours, so a contour corrected
here corrects the whole layer once; correcting the *beads* afterwards would leave
the interior — and every solid surface with it — sized to the uncorrected model.
It rewrites a layer's entire path list, which is only sound before any per-path
metadata exists; a `debug_assert!` pins that precondition.

- **Both corrections default to `0`, and `CompensationConfig::resolve` returns
  `None` then**, so the pass never allocates and the output is byte-identical to
  a build without it. That is what keeps the QA baselines stable — verify with
  the full gate after touching this module.
...
- **Non-goal:** hole-specific compensation. Holes move with the cross-section;
  giving them their own delta needs per-contour hole classification.
...
**Last Updated**: 2026-08-31 (dimensional compensation — medial-limited elephant foot; schema-driven settings taxonomy is now test-pinned)  
