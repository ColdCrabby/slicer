 ...
 * still has no cursor to hover with.
 */
...
/**
 * Tracks how much room the app has, and whether it has a cursor.
 *
 ...
 * | `isCoarsePointer` | is there a cursor to hover with? how big a target? |
 *
 * Target size follows the device, never the pointer in hand. iPadOS reports
 * `pointer: coarse` for a fingertip and a Pencil alike, and resizing the
 * interface on each `pointerType` change makes it flicker every time the user
 * swaps between the two — so a Pencil gets the fingertip sizes too.
 *
...
export class Viewport {
   ...
   * CSS reads the same answer through the `--touch-*` tokens in
   * `styles/base/_touch.scss`.
   */
