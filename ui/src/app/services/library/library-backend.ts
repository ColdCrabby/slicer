...
/** What a runtime's library can do — which is all the library page offers. */
export interface LibraryCapabilities {
  ...
  /** Whether a model's file can be shown in Finder or Explorer. */
...
export abstract class LibraryBackend {
  ...
  /** Where the library keeps its copies, where that is a folder on disk. */
  modelsDir(): Promise<string | null> {
    return Promise.resolve(null);
  }

  /** Show one of an entry's files in the system file manager. */
  reveal(_path: string): Promise<void> {
...
export class NativeLibraryBackend extends LibraryBackend {
  ...
  readonly capabilities: LibraryCapabilities = isTauriMobile()
    ? { modes: ['copy'], watchFolders: false, visibleFolder: true, reveal: false }
    : {
        modes: ['copy', 'reference', 'both'],
        watchFolders: true,
        visibleFolder: true,
        reveal: true,
      };
  ...
  override modelsDir(): Promise<string | null> {
    return this.#invoke('library_models_dir');
  }

  override reveal(path: string): Promise<void> {
    return this.#invoke('library_reveal', { path });
  }
